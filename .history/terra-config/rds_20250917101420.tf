...
resource "aws_security_group" "rds_sg" {
  ...
  ingress {
    ...
    to_port         = 3306
    protocol        = "tcp"
    security_groups = [aws_eks_node_group.example.resources[0].security_group_id]
  }
...
resource "aws_db_instance" "mysql" {
  ...
  db_name              = "threeTierDB"
  username             = "admin"
  password             = "Admin@123"  # Use variable or Secrets Manager in prod
...
resource "aws_security_group" "rds_sg" {
  ...
  ingress {
    ...
    to_port     = 3306
    protocol    = "tcp"
    # Restrict access to EKS Node Group SG
    cidr_blocks = []  # Placeholder, use CIDR if needed
  }
